...
function SpecialtyEditor({specialty}) {
    ...
    const [isUsersListVisible, setIsUsersListVisible] = React.useState(false)

    const [usersList, setUsersList] = React.useState(specialty.specialty_users ? specialty.specialty_users : [])
    const [usersIdList, setUsersIdList] = React.useState(specialty.specialty_users ? specialty.specialty_users.map(obj => obj.id) : [])
    const [availableUsersList, setAvailableUsersList] = React.useState([])
    ...
    const makeSpecsSearch = () => {
        getAllUsersByPartSecondName(usersInSearch).then(data => {
            setAvailableUsersList(data)
                    ...
                    <div className={'editor-search-item'}>
                        ...
                        <div className={'input-search-container'} onSubmit={(e) => makeSpecsSearch(e)}>
                            <input className={'editor-item-input'}
                                   ...
                                   type={'text'}
                                   value={usersInSearch}
                                   onChange={(e) => setUsersInSearch(e.target.value)}
                            />

                            <svg className={`input-search-hide ${isUsersListVisible ? 'visible' : 'hidden'}`}
                                 onClick={() => setIsUsersListVisible(false)}
                        ...
                        <div className={`input-error ${isUserInputValid > 0 ? 'hidden' : 'visible'}`}>
                            Поле не заполнено
                        </div>
                        <div className={`search-result-container ${isUsersListVisible ? 'visible' : 'hidden'}`}>
                            {availableUsersList && availableUsersList.length > 0 ? availableUsersList.map((item, index) => {
                                    return (
                                        <div className={'search-result enable'} key={index} onClick={() => {
                                            setIsUsersListVisible(false)
                                            if (!usersIdList.includes(item.id)) {
                                                setUsersList(specialtyList => [...specialtyList, item])
                                                setUsersIdList(specialtyList => [...usersIdList, item.id])
                                            }
                                        }}>
                                            {item.first_name + ' ' + item.second_name + ' ' + item.third_name + ' (' + item.email + ')'}
                                        </div>
                                    )
                                })
                                :
                        ...
                        {usersList && usersList.map((item, index) => {
                            return (<div className={'editor-selected-item'} key={index}>
                                ...
                                <svg className={'delete'} onClick={() => {
                                    setUsersList(usersList.filter(itemTmp => itemTmp !== item));
                                    setUsersIdList(usersIdList.filter(id => id !== item.id));
                                }}
                                     width="15" height="15" viewBox="0 0 15 15" fill="none"
                                     xmlns="http://www.w3.org/2000/svg">
                                    <path
